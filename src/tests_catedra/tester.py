...
def test(catedra_in, p):
	...
	start = time.time()
	res = subprocess.check_output("../tp1 {} {}; exit 0".format(catedra_in, p), shell=True).decode()
	end = time.time()
...
if (len(sys.argv) == 3):
	...
	tests = [('test_15_segundos', 0.9), ('test_30_segundos', 0.8), ('test_aleatorio', 0.85), ('test_aleatorio_desordenado', 0.76), ('test_trivial', 0.3), ('test_completo', 0.5), ('test_sin_links', 0.64)]
